...
Based on the Keras CIFAR-10 example and modified to have dropout only on the
dense layer, as shown in listing 5.13 of Deep Learning with Python (the cats
vs. dogs code).

It was created because the test/validation loss graphs for the network that
has dropout in all layers showed a high loss. This version will be used to
...
opt = tf.keras.optimizers.RMSprop(lr=args.learning_rate, decay=1e-6)
